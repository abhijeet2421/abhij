...
try {
    $conn = new PDO("sqlsrv:server = tcp:shahidawat.database.windows.net,1433; Database = shahidawat DB", "azure", "{JEET@123}");
...
$connectionInfo = array("UID" => "azure", "pwd" => "{your_password_here}", "Database" => "shahidawat DB", "LoginTimeout" => 30, "Encrypt" => 1, "TrustServerCertificate" => 0);
$serverName = "tcp:shahidawat.database.windows.net,1433";
$conn = sqlsrv_connect($serverName, $connectionInfo);

$YName = $_POST['YName'];
    $YEmail = $_POST['YEmail'];
    $YMobile = $_POST['YMobile'];
    $YDate = $_POST['YDate'];

    // Check if any of the input values are empty
    if (empty($YName) || empty($YEmail) || empty($YMobile) || empty($YDate)) {
        echo '<style>
        .modal-container {
            position: fixed;
            top: 0;
